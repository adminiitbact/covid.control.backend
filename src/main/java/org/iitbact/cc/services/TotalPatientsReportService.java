...
public class TotalPatientsReportService {
	...
	public List<String[]> fetchSumIsolationPatientList(String uid, CommonReportCriteria commonReportCriteria)
		...
		String qlString = "select admission.name, admission.admission_count, discharged.discharged_count " 
		        +"from ( " 
				...
				+ "where  facility.jurisdiction = ?"
				+ "group by  facility.name " + ") admission " 
				+ "LEFT join " + "("
				...
				+ "where discharged.reason=\"DISCHARGED\" and facility.jurisdiction = ?" + "group by facility.name " + ") discharged "
				+ "on admission.name=discharged.name";
